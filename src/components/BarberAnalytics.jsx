...
export default function BarberAnalytics() {
  ...
  const generateClosureText = () => {
    ...
    let text = `💈 *FECHAMENTO DE CAIXA — ${profile?.name || 'BARBEARIA ANDRADE'}*\n`;
    text += `📅 *Período:* ${rawDate ? `${rawDate} (${displayDate})` : displayDate}\n`;
    text += `✂️ *Barbeiro:* ${profile?.owner || 'Barbeiro'}\n`;
    text += `━━━━━━━━━━━━━━━━━━━━━\n`;
    ...
    text += `━━━━━━━━━━━━━━━━━━━━━\n`;
    text += `_Relatório emitido via App Barbearia Andrade_`;
    return text;
  };
  ...
  const handlePrintReport = () => {
    ...
    const printContent = `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <h1>${profile?.name || 'Barbearia Andrade'} — Fechamento Financeiro</h1>
            <p><strong>Barbeiro:</strong> ${profile?.owner || 'Barbearia'} | <strong>Contato:</strong> ${profile?.phone || ''}</p>
